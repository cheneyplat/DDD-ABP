...
namespace FirstABP.Tests
{
    public abstract class FirstABPTestBase : AbpIntegratedTestBase<FirstABPTestModule>
    {
        ...
        protected void UsingDbContext(Action<FirstABPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FirstABPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FirstABPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FirstABPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FirstABPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirstABPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FirstABPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirstABPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FirstABPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FirstABPDbContext, Task<T>> func)
        {
            T result;
